...
def update_expert_bias(model: nn.Module) -> None:
    ...
    Should be called once per optimizer step (after backward, before or after optimizer.step()).
    tokens_per_expert is zeroed after the update.
    """
    language_model = get_language_model(model)
    for transformer_block in language_model.layers:
        block_mlp = getattr(transformer_block, "mlp", None)
        if block_mlp is None or not hasattr(block_mlp, "load_balance_coeff"):
            continue
        if block_mlp.load_balance_coeff is None:
            continue
